...
locals {
  dor_fineos_etl_definition = templatefile("${path.module}/step_function/dor_fineos_etl.json",
    {
      app_name              = "pfml-api"
      cluster_arn           = data.aws_ecs_cluster.performance.arn
      ...
      sns_failure_topic_arn = "arn:aws:sns:us-east-1:498823821309:mass-pfml-performance-task-failure"
...
data "aws_security_group" "tasks" {
  name = "pfml-api-performance-tasks"
}
