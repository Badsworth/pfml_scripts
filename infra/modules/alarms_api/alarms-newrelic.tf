...
resource "newrelic_alert_policy" "service_desk_alerts" {
  name                = "PFML API Service Desk Alerts (${upper(var.environment_name)})"
  account_id          = local.newrelic_account_id
  incident_preference = "PER_CONDITION" # a new alarm will sound for every distinct alert condition violated
}
...
resource "newrelic_alert_policy_channel" "pfml_service_desk_alerts" {
  policy_id = newrelic_alert_policy.service_desk_alerts.id
  channel_ids = [
    newrelic_alert_channel.newrelic_service_desk_notifications.id
  ]
}
...
resource "newrelic_nrql_alert_condition" "notifications_endpoint_infinite_email_spam" {
  ...
  policy_id      = var.environment_name == "prod" ? newrelic_alert_policy.service_desk_alerts.id : newrelic_alert_policy.api_alerts.id
  type           = "static"
  value_function = "single_value"
  enabled        = true
