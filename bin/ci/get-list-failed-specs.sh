...
ENVIRONMENT=$1
TEST_RUN_ID=$2
QUERY="SELECT uniques(file) AS 'filename' FROM TestResult WHERE runId = '$TEST_RUN_ID' AND status = 'failed' AND environment = '$ENVIRONMENT' SINCE 1 week AGO UNTIL NOW"
