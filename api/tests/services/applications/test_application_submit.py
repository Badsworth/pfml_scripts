...
def test_submit_multiple_applications_same_employee_employer(user, test_db_session):
    ...
    assert len([call for call in capture if call[0] == "register_api_user"]) == 1
    assert (
        "register_api_user",
        None,
        {
            "employee_registration": massgov.pfml.fineos.models.EmployeeRegistration(
                ...
                employer_id=str(
                    massgov.pfml.fineos.mock.field.fake_customer_no(application_one.employer_fein)
                ),
                date_of_birth=date(1753, 1, 1),
                national_insurance_no=application_one.tax_identifier.tax_identifier,
            )
        },
    ) in capture
    assert len([call for call in capture if call[0] == "update_customer_details"]) == 1
    assert (
        ...
        fineos_user_id,
        {
            "customer": massgov.pfml.fineos.models.customer_api.Customer(
                ...
                idNumber=application_one.tax_identifier.tax_identifier,
                customerAddress=massgov.pfml.fineos.models.customer_api.CustomerAddress(
                    address=massgov.pfml.fineos.models.customer_api.Address(
                        addressLine1=application_one.residential_address.address_line_one,
                        addressLine2=application_one.residential_address.address_line_two,
                        addressLine4=application_one.residential_address.city,
                        addressLine6=application_one.residential_address.geo_state.geo_state_description,
                        postCode=application_one.residential_address.zip_code,
                        country="USA",
                    )
                ),
                gender="Female",
                classExtensionInformation=[
                    massgov.pfml.fineos.models.customer_api.ExtensionAttribute(
                        name="MassachusettsID", stringValue=application_one.mass_id
                    ),
                    massgov.pfml.fineos.models.customer_api.ExtensionAttribute(
    ...
    assert (
        len([call for call in capture if call[0] == "get_customer_occupations_customer_api"]) == 1
    )
    assert (
        "get_customer_occupations_customer_api",
        fineos_user_id,
        {"customer_id": application_one.tax_identifier.tax_identifier},
    ) in capture
            ...
            "contact_details": massgov.pfml.fineos.models.customer_api.ContactDetails(
                ...
                emailAddresses=[
                    massgov.pfml.fineos.models.customer_api.EmailAddressV20(
                        emailAddress=application_one.user.email_address
                    )
                ],
            )
        },
    ...
    assert (
        ...
        fineos_user_id,
        {
            "absence_case": massgov.pfml.fineos.models.customer_api.AbsenceCase(
                additionalComments="PFML API " + str(application_one.application_id),
    ...
    assert (
        ...
        {
            "absence_case": massgov.pfml.fineos.models.customer_api.AbsenceCase(
                additionalComments="PFML API " + str(application_two.application_id),
                intakeSource="Self-Service",
                notifiedBy="Employee",
                reason="Serious Health Condition - Employee",
                reasonQualifier1="Not Work Related",
...
def test_submit_multiple_applications_same_employee_different_employer(user, test_db_session):
    ...
    assert len([call for call in capture if call[0] == "register_api_user"]) == 2
