...
def test_upload_document_success(upload_1099_document_step: Upload1099DocumentsStep):
    ...
    ) as mock_get_tax_id, mock.patch(
        "massgov.pfml.fineos.client.AbstractFINEOSClient.upload_document_to_dms"
    ) as mocked_upload_document_to_dms, mock.patch(
        "massgov.pfml.delegated_payments.irs_1099.upload_documents.Upload1099DocumentsStep._retry_upload_document"
    ) as mocked_retry_upload_document:
        mock_get_tax_id.return_value = tax_identifier.tax_identifier
        mocked_upload_document_to_dms.response_value.ok = True
        mocked_upload_document_to_dms.response_value.text = "Success"
        mocked_retry_upload_document.return_value = None
...
def test_retry_upload_document_success(upload_1099_document_step: Upload1099DocumentsStep):
    batch = Pfml1099BatchFactory.create()
    employee = EmployeeFactory.create()
    tax_identifier = TaxIdentifierFactory.create()
    pfml_1099 = Pfml1099Factory.create(
    ...
    with mock.patch(
        ...
        response = upload_1099_document_step._retry_upload_document(
            client,
            os.path.join(os.path.dirname(__file__), "test_audit_batch.py"),
            pfml_1099.s3_location,
            DocumentType.IRS_1099G_TAX_FORM_FOR_CLAIMANTS.document_type_description,
...
def test_upload_1099_documents_flag_enabled_no_1099_records(
...
def test_upload_1099_documents_flag_enabled_invalid_s3_loaction(
    upload_1099_document_step: Upload1099DocumentsStep,
):
    with mock.patch(
        "massgov.pfml.delegated_payments.irs_1099.pfml_1099_util.is_upload_1099_pdf_enabled"
    ) as mocked_is_upload_1099_pdf_enabled, mock.patch(
        "massgov.pfml.delegated_payments.irs_1099.pfml_1099_util.get_current_1099_batch"
    ) as mocked_get_current_1099_batch:
        mocked_is_upload_1099_pdf_enabled.return_value = True
        mocked_get_current_1099_batch.return_value = None
        response = upload_1099_document_step._upload_1099_documents()
