...
def test_e2e_pub_payments(
    ...
    monkeypatch.setenv("FINEOS_VENDOR_MAX_HISTORY_DATE", "2021-04-30")
    monkeypatch.setenv("FINEOS_PAYMENT_MAX_HISTORY_DATE", "2021-04-30")
    ...
    with freeze_time("2021-05-01 19:00:00"):
        ...
        assert_payment_state_for_scenarios(
            test_dataset=test_dataset,
            scenario_names=[ScenarioName.OVERPAYMENT_PAYMENT,],
        ...
        payment_audit_report_outbound_folder_path = os.path.join(
            s3_config.payment_audit_report_outbound_folder_path, date_folder
        )

        payment_audit_report_sent_folder_path = os.path.join(
            s3_config.payment_audit_report_sent_folder_path, date_folder
        )

        audit_report_file_name = "Payment-Audit-Report.csv"

        assert_files(
            payment_audit_report_outbound_folder_path, [audit_report_file_name], timestamp_prefix
        )
        assert_files(
        ...
        audit_report_file_path = os.path.join(
            payment_audit_report_outbound_folder_path, f"{timestamp_prefix}{audit_report_file_name}"
        )

        audit_report_parsed_csv_rows = parse_csv(audit_report_file_path)

        assert (
            len(audit_report_parsed_csv_rows) == 3
        ...
        for report_name in PROCESS_FINEOS_EXTRACT_REPORTS:
            ...
            folder_name = report.folder_name.value
            file_name = f"{report.report_name.value}.csv"

            outbound_folder = os.path.join(s3_config.pfml_error_reports_path, folder_name)
            sent_folder = os.path.join(
                s3_config.pfml_error_reports_archive_path, folder_name, date_folder
            )

            assert_files(outbound_folder, [file_name], timestamp_prefix)
        ...
        assert_metrics(
            ...
            "PaymentExtractStep",
            {
                ...
                "errored_payment_count": 3,  # See DELEGATED_PAYMENT_ADD_TO_PAYMENT_ERROR_REPORT state check above
            },
        )
        ...
        assert_metrics(
            ...
            {
                "payment_sampled_for_audit_count": 3,  # See DELEGATED_PAYMENT_PAYMENT_AUDIT_REPORT_SENT state check above
            },
        )
