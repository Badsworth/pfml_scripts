...
def test_get_payments_200(
    ...
    payment_factory = DelegatedPaymentFactory(
        ...
        period_start_date=(date.today() + timedelta(weeks=1)),
        period_end_date=(date.today() + timedelta(weeks=3)),
        absence_period_start_date=date.today() - timedelta(days=1),
    )
...
def test_get_payments_200_payment_in_waiting_week(client, auth_token, user, test_db_session):
    absence_id = "NTN-12345-ABS-01"
    ...
    payment_factory = DelegatedPaymentFactory(
        ...
        period_end_date=(date.today() + timedelta(weeks=3)),
        # period_start_date - absence_period_start_date < 7
        # This means the payment generated by this factory
        # falls during the waiting week, so it will be ignored.
        absence_period_start_date=date.today() - timedelta(days=1),
    )
    claim = payment_factory.get_or_create_claim()
    ApplicationFactory.create(claim=claim, user=user)
    ...
    payment_response = response_body["payments"]

    assert len(payment_response) == 0
