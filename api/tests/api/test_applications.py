...
def test_applications_get_invalid(client):
    response = client.get("/v1/applications/{}".format("b26aa854-dd50-4aed-906b-c72b062f0275"))
...
def test_applications_post_start_app_unauthenticated(client):
    response = client.post("/v1/applications")
...
def test_application_patch(client, test_db_session):
    ...
    response = client.patch(
        "/v1/applications/{}".format(application.application_id), json=update_request_body,
    )
...
def test_application_patch_update_leave_period_belonging_to_other_application_blocked(
    ...
    response = client.get("/v1/applications/{}".format(application_2.application_id))
    response_body = response.get_json()
...
def test_application_patch_null_values(client):
    ...
    response = client.patch(
        "/v1/applications/{}".format(application.application_id), json=null_request_body,
    )
...
def test_application_patch_keys_not_in_body_retain_existing_value(client, test_db_session):
    ...
    response = client.patch(
        "/v1/applications/{}".format(application.application_id), json={"last_name": "Bar"},
    )
    assert response.status_code == 200
...
def test_application_patch_key_set_to_null_does_null_field(client, test_db_session):
    ...
    # null the field
    response = client.patch(
        "/v1/applications/{}".format(application.application_id), json={"first_name": None},
    )
    assert response.status_code == 200
