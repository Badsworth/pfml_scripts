...
import massgov.pfml.payments.data_mart_states_processing.vcc_error_report_sent as vcc_error_report_sent
...
def create_vcc_error_report_sent_state_log(test_db_session):
    state_log_setup = setup_state_log(
        associated_class=state_log_util.AssociatedClass.EMPLOYEE,
        start_states=[State.ADD_TO_VCC_ERROR_REPORT],
        end_states=[State.VCC_ERROR_REPORT_SENT],
...
def test_process_vcc_error_report_sent_no_vendor(
    test_db_session, mocker, initialize_factories_session, mock_data_mart_client,
):
    state_log = create_vcc_error_report_sent_state_log(test_db_session)
    ...
    # run process
    vcc_error_report_sent.process(test_db_session, mock_data_mart_client)
    ...
    assert new_state_log.start_state_id == State.VCC_ERROR_REPORT_SENT.state_id


def test_process_vcc_error_report_sent_mismatched_data(
    test_db_session, mocker, initialize_factories_session, mock_data_mart_client,
):
    state_log = create_vcc_error_report_sent_state_log(test_db_session)
    employee = state_log.employee
...
def test_process_vcc_error_report_sent_success(
    test_db_session, mocker, initialize_factories_session, mock_data_mart_client,
):
    state_log = create_vcc_error_report_sent_state_log(test_db_session)
    run_test_process_success_no_pending_payment(
        test_db_session, mock_data_mart_client, mocker, state_log, vcc_error_report_sent.process
    )
