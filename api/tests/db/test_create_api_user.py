...
def test_create_fineos_user_helper(test_db_session, monkeypatch):
    ...
    fineos_user = test_db_session.query(User).filter_by(active_directory_id="123abc").one()
    assert fineos_user is not None
    assert len(fineos_user.roles) == 1
    assert fineos_user.roles[0].role_id == Role.FINEOS.role_id

    internal_user = test_db_session.query(User).filter_by(active_directory_id="456def").one()
...
def test_create_fineos_user_helper_existing_user(
    ...
    monkeypatch.setenv("COGNITO_FINEOS_APP_CLIENT_ID", existing_user.active_directory_id)
    monkeypatch.setenv("COGNITO_INTERNAL_FINEOS_ROLE_APP_CLIENT_ID", "456def")
    ...
    fineos_user = (
        ...
        .filter_by(active_directory_id=existing_user.active_directory_id)
        .one()
    )
    assert fineos_user is not None
