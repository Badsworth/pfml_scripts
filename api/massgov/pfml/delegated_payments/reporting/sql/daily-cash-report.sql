...
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
SELECT 'Checks Payments' "Name", to_char(SUM(PBT.AMOUNT), '999,999,990D99') "Value"
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
SELECT 'Checks Payments' "Name", to_char(SUM(CASE WHEN PBT.CURRENT_STATE_ID IN (158) THEN 1 ELSE 0 END), '999,999,990') "Value"
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
SELECT 'Checks Payment Amount' "Name", to_char(SUM(CASE WHEN PBT.CURRENT_STATE_ID IN (158) THEN PBT.AMOUNT ELSE 0 END), '999,999,990D99') "Value"
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
SELECT 'ACH Payments' "Name", to_char(SUM(CASE WHEN PBT.CURRENT_STATE_ID IN (159) THEN 1 ELSE 0 END), '999,999,990') "Value"
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
SELECT 'ACH Payment Amount' "Name", to_char(SUM(CASE WHEN PBT.CURRENT_STATE_ID IN (159) THEN PBT.AMOUNT ELSE 0 END), '999,999,990D00') "Value"
FROM PAYMENT_BATCH_TRANSACTIONS PBT WHERE PBT.IS_CURRENT = 'Y'  AND PBT.CURRENT_STATE_ID IN (158, 159)
UNION ALL
