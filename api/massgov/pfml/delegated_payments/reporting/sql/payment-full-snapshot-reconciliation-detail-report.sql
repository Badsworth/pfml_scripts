...
SELECT 'MMARS Payment Reconciliation - Detail Transactions (through ' || CAST(DATE_TRUNC('WEEK', CURRENT_DATE) AS DATE)-2 || ')' TRANSACTION_DESCRIPTION, 
       NULL NTN, NULL I, NULL PAYMENT_DATE, NULL PERIOD_START_DATE, NULL PERIOD_END_DATE, NULL AMOUNT
UNION ALL
SELECT '* ' || 
       ...
       TO_CHAR(WARRANT_SELECT_DATE, 'MM/DD/YYYY') PAYMENT_DATE, TO_CHAR(PYMT_SERVICE_FROM_DATE, 'MM/DD/YYYY') PERIOD_START_DATE, TO_CHAR(PYMT_SERVICE_TO_DATE, 'MM/DD/YYYY') PERIOD_END_DATE, CAST(PYMT_ACTG_LINE_AMOUNT AS TEXT) AMOUNT
FROM MMARS_TXNS MT
LEFT OUTER JOIN FINEOS_TXNS FT ON MT.I = FT.I
LEFT OUTER JOIN FINEOS_CNCL FC ON FT.I = FC.I
LEFT OUTER JOIN FINEOS_RPLC FR ON FT.I = FR.I
...
SELECT '** ' || 
       ...
       TO_CHAR(PT.PAYMENT_DATE, 'MM/DD/YYYY') PAYMENT_DATE, TO_CHAR(PT.PERIOD_START_DATE, 'MM/DD/YYYY') PERIOD_START_DATE, TO_CHAR(PT.PERIOD_END_DATE, 'MM/DD/YYYY') PERIOD_END_DATE, CAST(PT.AMOUNT AS TEXT) AMOUNT
FROM PUB_TXNS PT
...
SELECT '*** ' || FT.TRANSACTIONST TRANSACTION_DESCRIPTION , FT.ABSENCECASENU NTN, FT.I,
       TO_CHAR(TO_DATE(FT.PAYMENTDATE, 'YYYY-MM-DD HH24:MI:SS'), 'MM/DD/YYYY') PAYMENT_DATE, TO_CHAR(P.PERIOD_START_DATE, 'YYYY-MM-DD') PERIOD_START_DATE, TO_CHAR(P.PERIOD_END_DATE, 'YYYY-MM-DD') PERIOD_END_DATE, CAST(FT.AMOUNT_MONAMT AS TEXT) AMOUNT
FROM FINEOS_TXNS FT 
