...
WITH PAYMENT_BATCH_TRANSACTIONS AS (
    SELECT P.PAYMENT_ID, P.FINEOS_EXTRACT_IMPORT_LOG_ID, P.PERIOD_START_DATE, P.PERIOD_END_DATE, P.PAYMENT_DATE,  P.AMOUNT, 
...
SELECT PBT.FINEOS_EXTRACT_IMPORT_LOG_ID,
       ...
       PBT.FINEOS_EXTRACTION_DATE "Fineos Extraction Date"FROM PAYMENT_BATCH_TRANSACTIONS PBT
INNER JOIN FINEOS_EXTRACT_VPEI VPEI ON PBT.FINEOS_EXTRACT_IMPORT_LOG_ID = VPEI.FINEOS_EXTRACT_IMPORT_LOG_ID
                                   AND PBT.FINEOS_PEI_C_VALUE = VPEI.C
                                   AND PBT.FINEOS_PEI_I_VALUE = VPEI.I
LEFT OUTER JOIN EMPLOYER EMPR ON EMPR.EMPLOYER_FEIN = VPEI.PAYEESOCNUMBE
WHERE PBT.IS_CURRENT = 'Y'
  AND PBT.CURRENT_STATE_ID IN (149, 150)
-- Add employer reimbursement payment to FINEOS Writeback
-- Employer reimbursement payment FINEOS Writeback sent