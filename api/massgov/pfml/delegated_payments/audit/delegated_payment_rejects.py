...
class PaymentRejectsStep(Step):
    ...
    def transition_audit_pending_payment_state(
        ...
        if (
            payment_state_log.end_state_id not in [State.DELEGATED_PAYMENT_PAYMENT_AUDIT_REPORT_SENT.state_id, State.STATE_WITHHOLDING_PENDING_AUDIT.state_id, State.FEDERAL_WITHHOLDING_PENDING_AUDIT.state_id]
        ):
        ...
        if payments_util.is_withholding_payments_enabled():
            ...
            if payment.payment_transaction_type_id in [PaymentTransactionType.STATE_TAX_WITHHOLDING.payment_transaction_type_id,PaymentTransactionType.FEDERAL_TAX_WITHHOLDING.payment_transaction_type_id]:
                if is_rejected_payment:
                    ...
                    outcome = state_log_util.build_outcome(f"Payment rejected with notes: {rejected_notes}")
        ...
        if payment.payment_transaction_type_id not in [PaymentTransactionType.STATE_TAX_WITHHOLDING.payment_transaction_type_id,PaymentTransactionType.FEDERAL_TAX_WITHHOLDING.payment_transaction_type_id]:
            if is_rejected_payment:
                self.increment(self.Metrics.REJECTED_PAYMENT_COUNT)
                state_log_util.create_finished_state_log(
                    payment,
                ...
                if payments_util.is_withholding_payments_enabled():
                    ...
                    if withholding_records:
                        self.set_statelog_for_withholding_linksplit_payments(
                            withholding_records, is_rejected=True, is_skipped=False, rejected_notes=rejected_notes
                        )
                ...
                if payments_util.is_withholding_payments_enabled():
                    ...
                    if withholding_records:
                        self.set_statelog_for_withholding_linksplit_payments(
                            withholding_records, is_rejected=False, is_skipped=True, rejected_notes=rejected_notes
                        )
            else:
                self.increment(self.Metrics.ACCEPTED_PAYMENT_COUNT)
                state_log_util.create_finished_state_log(
    ...
    def set_statelog_for_withholding_linksplit_payments(
        ...
        for payment_withholding_record in withholding_records:
            ...
            if is_skipped:
                end_state = (
                    State.STATE_WITHHOLDING_ADD_TO_PAYMENT_REJECT_REPORT_RESTARTABLE
                    if (withhold_payment.payment_transaction_type_id == PaymentTransactionType.STATE_TAX_WITHHOLDING.payment_transaction_type_id)
                    else State.FEDERAL_WITHHOLDING_ADD_TO_PAYMENT_REJECT_REPORT_RESTARTABLE
                )
    ...
    def _manage_pei_writeback_state(self, payment: Payment, rejected_notes: Optional[str] = None) -> None:
        ...
        logger.info("writeback for WITHHOLDING rejected and skipped %s",writeback_transaction_status.transaction_status_description)
        writeback_details = FineosWritebackDetails(
            payment=payment,
            transaction_status_id=writeback_transaction_status.transaction_status_id,
            import_log_id=self.get_import_log_id(),
