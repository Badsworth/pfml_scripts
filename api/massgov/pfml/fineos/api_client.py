...
class RestApiCaller(abc.ABC, metaclass=abc.ABCMeta):
    pass

class OauthRestApiCaller(RestApiCaller):
    oauth2_url: str
    client_id: str
    client_secret: str
    ...
    def make_request(self, method: str, url: str, request_timeout: float, headers: Dict[str, str], **args: Any):
        try:
...
class FineosExceptionHandler():
    def request_log_attributes(self, method, url, error, method_name) -> Dict[any, any]:
        has_flask_context = flask.has_request_context()
        return {
            "fineos.error.class": type(error).__name__,
            "fineos.error.message": str(error),
    ...
    def response_log_attributes(self, response, error) -> Dict[any, any]:
    ...
    # FINEOS Exception Handler
    def handle_exception(self, method: str, url: str, error: Exception, method_name: str):
        
        newrelic.agent.record_custom_event(
            "FineosError",
            self.request_log_attributes(method, url, error, method_name)
        )

    def handle_response_exception(self, response, error, method, url, method_name):
        has_flask_context = flask.has_request_context()
        ...
        newrelic.agent.record_custom_event(
            ...
            {
                **self.request_log_attributes(method, url, error, method_name),
                **self.response_log_attributes(response, error)
            },
        )
...
class ApiClient():
    ...
    def __init__(
        ...
        # self.caller = OauthRestApiCaller
        self.request_count = 0
        logger.info(
            "customer_api_url %s, wscomposer_url %s, group_client_api_url %s, "
            "integration_services_api_url %s",
    ...
    def _handle_client_side_exception(
        ...
        # return an error.
        exception_handler.handle_exception(method, url, ex, method_name)
    ...
    def _request(
        ...
        if response.status_code != requests.codes.ok:
            ...
            log_fn(
                ...
                extra=exception_handler.response_log_attributes(response, err),
                exc_info=err,
            )

            exception_handler.handle_response_exception(err)
            raise err
