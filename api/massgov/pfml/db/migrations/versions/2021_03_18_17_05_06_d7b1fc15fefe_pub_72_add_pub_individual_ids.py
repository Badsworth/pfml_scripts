...
def upgrade():
    ...
    op.execute(CreateSequence(Sequence("payment_individual_id_seq", start=1)))
    op.add_column(
        "payment",
        sa.Column(
            "pub_individual_id",
    ...
    op.execute(CreateSequence(Sequence("pub_eft_individual_id_seq", start=1)))
    op.add_column(
        "pub_eft",
        sa.Column(
            "pub_individual_id",
...
def downgrade():
    ...
    op.execute(DropSequence(Sequence("pub_eft_individual_id_seq")))
    op.drop_index(op.f("ix_payment_pub_individual_id"), table_name="payment")
    op.drop_column("payment", "pub_individual_id")
    op.execute(DropSequence(Sequence("payment_individual_id_seq")))
