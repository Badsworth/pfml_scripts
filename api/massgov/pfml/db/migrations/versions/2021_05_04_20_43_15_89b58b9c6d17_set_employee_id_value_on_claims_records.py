...
def upgrade():
    ...
    tax_identifier_ids_to_claim_ids = {
        row.tax_identifier_id: row.claim_id for row in applications_claims_and_tax_identifier_ids
    }
    ...
    tax_identifier_ids_to_employees = {
        row.tax_identifier_id: row.employee_id for row in tax_identifier_ids_to_employees_results
    }
    ...
    while current_batch < total_batches:
        connection.execute(
            stmt, bulk_update_params[current_batch * batch_size : (current_batch + 1) * batch_size]
        )
        current_batch += 1
