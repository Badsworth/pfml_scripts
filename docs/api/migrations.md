...
Database migrations are stored under [api/massgov/pfml/db/migrations](/api/massgov/pfml/db/migrations/).

## Tooling

The API runs on top of a PostgreSQL database, using SQLAlchemy as the Object-Relational Model (ORM) layer and Alembic as the data migration tool.
...
Since the database and API are independent services, it is critical that engineers understand how database and API changes are deployed. ...
