...
const pathsToDocumentTypes = {
  [uploadRoutes.bondingAdoptionStatement]:
...
const CertificationUpload = ({ path }) => {
  const context = {
    [uploadRoutes.bondingAdoptionStatement]: "bonding_adopt_foster",
    [uploadRoutes.bondingProofOfBirth]: "bonding_newborn",
    [uploadRoutes.caringCertification]: "care",
    [uploadRoutes.medicalCertificationOfCondition]: "medical",
    [uploadRoutes.pregnancyMedicalCertification]: "medical",
  }[path];
  const isBondingAdoption = path === uploadRoutes.bondingAdoptionStatement;
  const isBonding =
    ...
    <React.Fragment>
      <Heading level="2" size="1">
        {t("pages.claimsUploadCertification.sectionLabel", {
          context: isBonding ? "bonding" : null,
        })}
      </Heading>
      <Lead>
        <Trans
...
const IdentificationUpload = ({ path }) => {
  ...
  return (
    <React.Fragment>
      <Heading level="2" size="1">
        {t("pages.claimsUploadId.sectionLabel", {
          context: isStateId ? "mass" : "other",
        })}
      </Heading>
      ...
      {!isStateId && (
        <div className="border-bottom border-base-light margin-bottom-4 padding-bottom-4">
          ...
          <Accordion>
            <AccordionItem heading={t("pages.claimsUploadId.accordionHeading")}>
              <Trans
...
const DocumentUpload = (props) => {
  ...
  const path = `/applications/upload/${query.documentType}`;
  ...
  return (
    <QuestionPage
      title={
        isIdUpload
          ? t("pages.claimsUploadId.title")
          : t("pages.claimsUploadCertification.title")
      }
      onSave={handleSave}
    >
