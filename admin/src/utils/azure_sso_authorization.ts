...
export const authorizeUser = async (
  router: NextRouter,
  setUser: React.Dispatch<React.SetStateAction<api.UserResponse | null>>,
...
export const getAccessToken = (
  ...
  const authURIRes: api.AuthURIResponse = JSON.parse(
    localStorage.getItem(SSO_AUTH_URI) || "{}",
  );

  if (router.query.code && "state" in authURIRes) {
    ...
    const authCodeRes: api.AuthCodeResponse = router.query;
