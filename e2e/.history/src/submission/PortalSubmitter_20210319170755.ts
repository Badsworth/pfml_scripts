...
export default class PortalSubmitter {
  ...
  async submit(
    ...
    const g = await this.uploadDocuments(
      application_id,
      claim.documents,
      options
    );
    console.log("DOCS UPLOADED", g);
  ...
  protected async uploadDocuments(
    ...
    try {
      ...
      // console.log("DOCUMENTS PROMISIFIED", promises);
      const result = await Promise.all(promises);
    } catch (e) {
      console.log("ERROR UPLOAD", e);
      e.data.errors &&
        e.data.errors.map((ex: string) => console.log("ERROR OBJ", ex));
    }
  }
