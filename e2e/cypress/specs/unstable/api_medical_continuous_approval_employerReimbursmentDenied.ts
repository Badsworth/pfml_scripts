...
(config("HAS_EMPLOYER_REIMBURSEMENTS") === "true"
  ? describe
  : describe.skip)("Employer Reimbursement Denial", () => {
  after(() => {
  ...
  it("CSR Denies the Employer Reimbursement Process", () => {
    ...
    cy.unstash<DehydratedClaim>("claim").then((claim) => {
      cy.unstash<Submission>("submission").then((sumbmission) => {
        ...
        claimPage.addCorrespondenceDocument("Employer Reimbursement Formstack");
        ...
        claimPage.documents((document) => {
          document.setDocumentComplete("Employer Reimbursement Denial Notice");
        });
