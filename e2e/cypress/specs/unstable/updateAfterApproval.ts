...
describe("Post-approval (notifications/notices)", { retries: 0 }, () => {
      ...
      cy.unstash<Submission>("submission").then((submission) => {
        ...
        cy.unstash<ApplicationRequestBody>("claim").then((claim) => {
          ...
          cy.task<Email[]>(
            ...
            for (const emailSingle of emails) {
              email
                .getNotificationData(emailSingle.html)
                .then((data: { [key: string]: string }) => {
                  if (
                    data.applicationId.includes(submission.fineos_absence_id)
                  ) {
  ...
  // Update approved claim in Fineos
  it(
    "In Fineos, update the approved claim",
    { baseUrl: getFineosBaseUrl() },
    () => {
      beforeFineos();
      bailIfThisTestFails();
