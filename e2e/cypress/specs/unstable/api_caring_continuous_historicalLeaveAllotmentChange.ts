...
describe("Create a new caring leave claim in API submission and add Historical Absence case", () => {
  ...
  const leaveallotment =
    ...
    cy.unstash<Submission>("submission").then((submission) => {
      ...
      claimPage.documents((docs) => {
        ...
        docs.adjustDocumentStatus("Leave Allotment Change Notice", "Completed")
        docs.checkDocumentFileExtension("Leave Allotment Change Notice", /\.pdf$/)
      })
      claimPage.triggerNotice("Leave Allotment Change Notice");
    });
  });

  it("Check the Claimant email for the Leave Allotment Change notification",
    { retries: 0 },
    () => {
  ...
  it("Check the Leave Admin email for the Leave Allotment Change notification",
    { retries: 0 },
    () => {
      cy.dependsOnPreviousPass([historical, leaveallotment]);
    cy.unstash<Submission>("submission").then((submission) => {
