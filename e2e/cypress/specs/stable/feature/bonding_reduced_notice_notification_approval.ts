            ...
            cy.task("generateClaim", "REDUCED_ER").then((claim) => {
                ...
                cy.task("submitClaimToAPI", {
                    ...
                    claimPage.adjudicate((adjudication) => {
                        ...
                        adjudication.certificationPeriods((cert) =>
                            cert.prefill()
                        );
                        adjudication.acceptLeavePlan();
                    });
                    claimPage.tasks((tasks) => {
                        const certificationDoc = findCertificationDoc(
                            claim.documents
                        );
                ...
                cy.unstash<ApplicationRequestBody>("claim").then((claim) => {
                    email
                        .getEmails(
                            {
                                address:
                                    "gqzap.notifications@inbox.testmail.app",
        ...
        () => {
            ...
            cy.unstash<Submission>("submission").then((submission) => {
                cy.unstash<ApplicationRequestBody>("claim").then((claim) => {
                    ...
                    cy.contains(submission.fineos_absence_id);
                    cy.get(
                        `a[href*="${config("PORTAL_BASEURL")}/applications"]`
                    );
                });
            });
        }
