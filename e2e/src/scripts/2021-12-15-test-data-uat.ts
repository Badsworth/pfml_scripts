...
import * as scenarios from "../scenarios/2021-12-15-test-data-uat";

(async () => {
  // @todo Rename the data directory as needed.
  const storage = dataDirectory("2021-12-16-test-data-uat-batch3");
  // <!-- @default
  ...
  try {
    ...
    (employerPool = EmployerPool.generate(2)),
  ...
  try {
    ...
    employeePool = EmployeePool.merge(
      ...
      EmployeePool.generate(5, employerPool, { wages: "eligible" }),
      EmployeePool.generate(5, employerPool, { wages: 90000 }),
    );
    // <!-- @default
