 ...
 */
(async () => {
  const storage = dataDirectory("END-727-50k");
  ...
  try {
    ...
    (employerPool = EmployerPool.generate(50000, { size: "small" })),
  ...
  try {
    ...
    employeePool = EmployeePool.merge(
      EmployeePool.generate(450000, employerPool, {
        wages: "eligible",
      }),
      EmployeePool.generate(50000, employerPool, {
        wages: "ineligible",
      })
    );
