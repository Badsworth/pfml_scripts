...
(async () => {
  const storage = dataDirectory("2021-05-17-pfmlpb-1212");
  ...
  try {
    ...
    (employerPool = EmployerPool.merge(
      EmployerPool.generate(30, { size: "small" }),
      EmployerPool.generate(15, {
        withholdings: [0, 0, 0, null],
        size: "small",
      }),
      EmployerPool.generate(5, { withholdings: [0, 0, 0, 0], size: "small" })
    )),
  ...
  try {
    ...
    employeePool = EmployeePool.merge(
      EmployeePool.generate(900, employerPool, { wages: "eligible" }),
      EmployeePool.generate(100, employerPool, { wages: "ineligible" })
    );
