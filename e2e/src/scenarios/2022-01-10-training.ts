...
const generatetMaxLeave = (maxLeaveLength: 20 | 12): [Date, Date] => {
  const start = faker.date.between(
    parseISO("2021-12-27"),
    parseISO("2022-03-01")
  );
  const end = addWeeks(start, maxLeaveLength);
  return [start, end];
};

const GENERATE_LEAVE_DATES_DESCRIPTION =
  "Leave begins between 12/12/21 - 03/01/22\nEnds 4-12 weeks after start date";
...
export const TRND: ScenarioSpecification = {
  ...
  claim: {
    ...
    leave_dates: () => generatetMaxLeave(20),
  },
};
